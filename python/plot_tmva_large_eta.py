...
def ExtractBGRejection(S, Energies):
    ...
    for e in Energies:
        ene = str(e)
        file = TrainPath+TrainType+ene+unit+test+".root"
        print file
        infile = r.TFile.Open(file)
        dir = TrainType+ene+unit+test+"/"+"Method_"+TrainMethod+TrainType+ene+unit+test+"/"+TrainMethod+TrainType+ene+unit+test
        hist = infile.Get(dir+"/MVA_"+TrainMethod+TrainType+ene+unit+test+"_rejBvsS")
        hist.Draw()
        x = hist.GetXaxis()
        ID = x.FindBin(S)
...
gr1.SetLineColorAlpha(r.kBlue-9,0.23)
...
gr2.SetLineWidth(2)
gr2.SetLineColorAlpha(r.kOrange-2,0.1)
...
#axis.Draw()
leg = r.TLegend(0.50,0.70,0.88,0.88)
leg.SetFillColorAlpha(r.kYellow-10,0.03)
